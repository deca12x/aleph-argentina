...
import { EphemeralMessage, MessageSender } from "@/lib/types";
...
export default function EphemeralChat() {
  ...
  const isEntryPage =
    ...
    !pathname.includes("reallife");
  const hasEnteredSpace =
    pathname.includes("/clans/") &&
    (pathname.includes("matrix") || pathname.includes("reallife"));
  ...
  const secondaryColor =
    currentClan?.visualProperties?.secondaryColor || "#FF9800";
  ...
  useEffect(() => {
    ...
    const placeholders = Array(emptySlots > 0 ? emptySlots : 0)
      .fill(null)
      .map((_, i) => ({
        id: `empty-${i}`,
        text: "",
    ...
    if (topMessages.length >= MAX_WALL_MESSAGES) {
      const lowestPayment =
        parseFloat(topMessages[topMessages.length - 1].paymentAmount) || 0;
  ...
  const handleSendMessage = async () => {
    ...
    if (isWallFull && amount < parseFloat(minPaymentToReplace)) {
      alert(
        `The wall is full. To replace a message, you need to pay more than ${minPaymentToReplace} ${tokenSymbol}`
      );
      return;
    }
      ...
      if (success) {
        ...
        setTimeout(() => {
          ...
          window.dispatchEvent(
            new CustomEvent("newBlockchainMessage", {
              detail: {
                id: msgId,
            ...
            setWallMessages((prev) =>
              [
                ...prev,
                {
                  ...
                  timestamp: new Date(),
                  expiresAt: new Date(
                    Date.now() + messageDuration(paymentAmount) * 60000
                  ),
                  paymentAmount: paymentAmount,
                },
              ].sort(
                (a, b) =>
                  parseFloat(b.paymentAmount) - parseFloat(a.paymentAmount)
              )
            );
          } else {
  ...
  return (
    <div
      className={`fixed bottom-4 right-4 z-[1000] flex flex-col transition-all duration-300 ease-in-out ${
        isExpanded
          ? "w-[320px] md:w-[420px] shadow-2xl"
          : "w-[280px] md:w-[320px] shadow-lg"
      }`}
    >
            ...
            <div className="flex items-center text-white">
              ...
              <div className="text-xs">
                {wallMessages.length < MAX_WALL_MESSAGES ? (
                  <span>
                    Wall has {wallMessages.length}/{MAX_WALL_MESSAGES} messages.
                    You can post for free!
                  </span>
                ) : (
                  <span>
                    Wall is full! Min payment to replace: {minPaymentToReplace}{" "}
                    {tokenSymbol}
                  </span>
                )}
              </div>
                ...
                visibleMessages.map((message) => (
                  <motion.div
                    ...
                    className={`rounded-lg p-2 border ${
                      message.id === lastMessageId ? "ring-2" : ""
                    }`}
                    style={{
                      background:
                        parseFloat(message.paymentAmount) > 10
            ...
            {authenticated ? (
              <div className="p-3">
                <div className="flex items-end">
                  ...
                  <button
                    className={`ml-2 bg-white/10 ${
                      isSubmitting || txStatus === "confirming"
                        ? "opacity-50 cursor-not-allowed"
                        : "hover:bg-white/20 cursor-pointer"
                    } text-white p-2 rounded-lg transition-colors`}
                    onClick={() =>
                      inputMessage.trim() && setShowPaymentInput(true)
                    }
                    disabled={isSubmitting || txStatus === "confirming"}
                  >
                    <Send size={16} />
                ...
                {!hasEnteredSpace &&
                  inputMessage.trim() &&
                  showPaymentInput && (
                    <div className="mt-3 p-3 rounded-lg border border-yellow-500/30 bg-yellow-500/10 backdrop-blur-lg">
                      <div className="flex items-start mb-2">
                        <AlertCircle
                          size={16}
                          className="text-yellow-400 flex-shrink-0 mr-2 mt-0.5"
                        />
                      ...
                      <p className="text-xs text-white/90 mb-2">
                        You must enter the clan space to write messages on this
                        public wall.
                        {isEntryPage
                          ? " Click the 'Enter Space' button below."
                          : " Navigate to a clan first."}
                      </p>
                      <ul className="text-xs text-white/80 list-disc pl-4 mb-2 space-y-1">
                        ...
                        <li>
                          When wall is full, new bids will replace the lowest
                          paid messages
                        </li>
                        <li>
                          Set a higher price to ensure your message stays
                          visible
                        </li>
                      </ul>
                      {isEntryPage ? (
                      ...
                      <div className="mb-3 text-xs text-white/70">
                        {wallMessages.length > 0 ? (
                          wallMessages.length >= MAX_WALL_MESSAGES ? (
                            ...
                            parseFloat(minPaymentToReplace) ? (
                              <span className="text-green-300">
                                Your message will replace the cheapest message
                                in the wall.
                              </span>
                            ) : (
                              <span className="text-yellow-300">
                                Not enough to replace any messages. Increase
                                your payment.
                              </span>
                            )
                          ) : (
