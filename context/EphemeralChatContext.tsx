...
import { EphemeralMessage } from "@/lib/types";
  ...
  const fetchMessages = async () => {
    try {
      ...
      const response = await fetch("/api/ephemeral-chat", {
        ...
        console.warn(
          "Ephemeral chat fetch failed, using local data instead:",
          error
        );
        return null;
      });
      ...
      if (response && response.ok) {
        ...
        if (data.messages) {
          ...
          const formattedMessages: EphemeralMessage[] = data.messages.map(
            (msg: any) => ({
              ...msg,
        ...
        // If response is not ok, fall back to local storage
        throw new Error("API response not ok");
      }
    } catch (err) {
      console.warn("Error fetching ephemeral messages:", err);
  ...
  const loadFromLocalStorage = () => {
    ...
    if (storedMessages) {
      try {
        ...
        const parsedMessages: EphemeralMessage[] = JSON.parse(
          storedMessages,
          (key, value) => {
    ...
    if (
      ...
      (!storedMessages || JSON.parse(storedMessages).length === 0)
    ) {
      addSampleMessages();
    }
  };
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      ...
      setMessages((prevMessages) =>
        prevMessages.filter((msg) => msg.expiresAt > now)
      );
